...
:::beispiel
In einer Geschwindigkeitsmessung mit dem Seitenradar an der Universitätsstraße in Bochum wurden innerhalb von zwei Tagen die Geschwindigkeiten von `r nrow(d_unistrasse)` Fahrzeugen gemessen. 
...
d_unistrasse |>
  ...
  gt_preview() |>
  tab_source_note(
    source_note = md("Daten bereitgestellt von Iris Mühlenbruch")
  ) |>
...
#| fig-cap: Quantile und Median einer Dichtekurve
...
Der Vorfaktor $1 / (\sigma \sqrt{2\pi})$ in der Funktionsgleichung von $f$ sorgt dafür, dass die Fläche unter dem Graphen von $f$ gleich 1 ist. Interessanterweise besitzt die Normalverteilung keine Stammfunktion, so dass man numerisch integrieren muss (zum Glück gibt es Computer). Für die Anteile in bestimmten Intervallen um den Mittelwert $\mu$ gilt die folgende Regel:
...
$$
\begin{aligned}
    68\% & \quad \text{im Intervall} \quad \mu \pm \sigma \\[1em]
    95\% & \quad \text{im Intervall} \quad \mu \pm 2\sigma \\[1em]
    99.7\% & \quad \text{im Intervall} \quad \mu \pm 3\sigma
\end{aligned}
$$
:::
::: {.column width=50%}
![](00-bilder/normalverteilung-prozentregel.svg){fig-align="right" width="90%"}
:::
::::
:::::
...
Für die Geschwindigkeitsmessung auf Universitätsstraße erhalten wir die Kenngrößen 
...
Für die Geschwindigkeitsmessung an der Universitätsstraße stellt die Normalverteilung nur eine unzureichende Approximation der wirklichen Verteilung dar. Das sieht man auch an dem NQ-Plot in @fig-unistrasse-nq.

```{r}
#| label: fig-unistrasse-nq
#| fig-cap: "Da die wirkliche Verteilung deutlich steiler ist als die angepasste Normalverteilung (@fig-normalverteilung-angepasst) ergibt sich der hierfür charakteristische Verlauf der Punkte: Kleine Werte liegen unterhalb der Geraden, große Werte oberhalb."
...
Die Idee dabei ist es, eine Kernfunktion $K : \mathbb{R} \to \mathbb{R}$ zu definieren und diesen Kern über die Datenpunkte zu schieben. An jeder Stelle $x$ werden die Längen der Linien von den Werten $x_i$ bis zur Kernfunktion addiert und durch die Anzahl der Werte $n$ geteilt (siehe @fig-approximation-dichtekurve).



![Approximation einer Dichtekurve](00-bilder/approximation-dichtekurve.svg){#fig-approximation-dichtekurve}

Am gebräuchlichsten Kerne mit den Funktionsgleichungen 
...
Für alle Kerne gilt, dass die vom zugehörigen Graphen überdeckte Fläche gleich 1 ist. Damit können wir den sogenannten Kern-Dichteschätzer definieren.

::: definition
**Definition: Kern-Dichteschätzer**

Zu einer Kernfunktion $K : \mathbb{R} \to \mathbb{R}$, gegebenen
...
ein [Kern-Dichteschätzer]{.neuerbegriff} für die unbekannte Dichtekurve $f$. Dabei legt der Parameter $h$ die Breite des Kerns fest.
:::
