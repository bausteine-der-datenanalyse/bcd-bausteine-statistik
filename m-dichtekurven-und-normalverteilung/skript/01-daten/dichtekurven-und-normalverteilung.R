...
fig_kerndichteschaetzer <- function() {
    ...
    kernel <- \(u) if_else(abs(u) <= 1, 0.75 * (1 - u^2), 0)
    kernel_s <- \(x) 1 / h * kernel((x - xs) / h)
    fhat <- Vectorize(\(x) 1 / (n * h) * sum(kernel((x - pull(d, x)) / h)))
    ...
    ggplot(data = d, aes(x = x)) +
        ...
        annotate(
            ...
            x = -1.4,
            y = 0.9,
            label = "Kernfunktion kernel für die Stelle x",
            hjust = 0,
        ) +
        theme_void()
}
