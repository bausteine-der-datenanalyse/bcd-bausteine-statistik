...
```{r}
tbl_wirtschaftswachstum(1975, 1987)
```

```{r}
tbl_wirtschaftswachstum(1987, 2000)
```
...
Wie lässt sich nun eine solche Zahl bestimmen? Die grundlegende Idee besteht darin, dass im Streudiagramm ein Koordinatensystem in das Zentrum der Punktwolke gelegt wird. Für jeden Punkt $(x_i, y_i)$ lässt sich dann überprüfen, in welchem Quadrant des neuen Koordinatensystems der Punkt liegt. In @fig-korrelation ist dieser Zusammenhang farblich hervorgehoben. Wir sehen sofort:
...
Entsprechend dem Zahlenwert von $r$ unterscheidet man grob die folgenden Korrelationsgrade:
