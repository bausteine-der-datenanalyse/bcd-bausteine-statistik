...
ggplot(
  ...
  geom_abline(intercept = 0, slope = 1, color = "red", linewidth = 0.75) +
  geom_point(shape = 21, fill = "light blue", size = 2.5) +
  geom_rug() +
...
ggplot(data = arrange(d3, gdppc), mapping = aes(x = he, y = le)) +
  geom_point(
    mapping = aes(fill = gdppc, size = gdppc),
    shape = 21,
    show.legend = F,
    alpha = 0.8
  ) +
  ...
  labs(x = "Ausgaben Gesundheitswesen (% BIP)", y = "Mittlere Lebenserwartung")
```
...
p0 <- ggplot(data = d3, mapping = aes(x = he, y = le))
...
p5 <- plot_correlation_example(
  x,
  -1 + 2 * x^2 + rnorm(n, sd = 0.3),
  TeX("E: $r \\approx 0$")
)
p6 <- plot_correlation_example(
  x,
  1 - 2 * x^2 + rnorm(n, sd = 0.3),
  TeX("F: $r \\approx 0$")
)
...
ggplot(data = d5, mapping = aes(x = read, y = math)) +
  geom_point(mapping = aes(fill = sel), shape = 21, show.legend = FALSE) +
  geom_label(
...
sondern es kommt noch eine zufällige Abweichung hinzu.
...
lineare Einfachregression. Dabei ist $\alpha$ der $y$-Achsenabschnitt, $\beta$ die Steigung und
...
ggplot(data = d_pisa, mapping = aes(x = read, y = math)) +
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE, color = "red", linewidth = 0.75) +
  geom_point(shape = 21, fill = "light blue", size = 2.5) +
  labs(x = "Lesekompetenz", y = "Mathematische Kompetenz")
```
:::
...
```{r}
plot_rsquare_example(y1) +
  plot_rsquare_example(y2) +
  plot_layout(axes = "collect") &
