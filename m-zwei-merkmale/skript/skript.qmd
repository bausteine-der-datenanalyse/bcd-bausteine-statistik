...
Varianten sind in @fig-histogramme-2d zu sehen.

```{r}
p5 <- ggplot(data = d3) +
  stat_bin2d(mapping = aes(x = he, y = le), binwidth = c(2.5,5)/2, color = "black", linewidth = 0.25) +
...
p6 <- ggplot(data = d3) +
  stat_density_2d(mapping = aes(x = he, y = le, fill = after_stat(level)), color = "black", linewidth = 0.25, geom = "polygon", show.legend = FALSE, alpha = 0.8, bins = 10) +
  labs(x = "Gesundheitswesen (% BIP)", y = "Mittlere Lebenserwartung", fill = "Anzahl") +
...
p8 <- wrap_elements(
  panel = rasterGrob(readPNG("00-bilder/weltbank-3d.png"), interpolate = TRUE)
)
```

```{r}
#| fig-cap: "Darstellung eines Histogramms (links) und einer geschätzten Verteilungsfunktion (rechts) für Lebenserwartung und Ausgaben für das Gesundheitswesen. Die 3D-Plots wurden mit dem Programm Mathematica erstellt, die 2D-Plots in R. Offenbar werden unterschiedliche Klasseneinteilungen für das Histogramm verwendet"
#| label: fig-histogramme-2d
(p5 + p6) /
free(p8)
```
