...
source("01-daten/statistische-daten.R")
```
...
- Ministerien (etwa [BMV](https://bmv.de/SharedDocs/DE/Artikel/G/verkehr-in-zahlen.html) oder [BMWE](https://www.bundeswirtschaftsministerium.de/Navigation/DE/Themen/themen.html?cl2Categories_LeadKeyword=energiedaten-und-szenarien)) und Bundesämter ([Bundesanstalt für Straßen- und Verkehrswesen](https://www.bast.de), [Statistisches Bundesamt](https://www.destatis.de)).
...
#| fig-alt: |
#|  Zeitraum von 100 Jahren auf der X-Achse, Abfluss gemessen in m³/s auf der Y-Achse.
#|  Jeder Tag stellt einen Datenpunkt dar."
...
d_tidydata_41 |>
  tab_tidydata() |>
  tab_spanner(label = "Fälle", columns = c("1999", "2000"))
```

```{r}
d_tidydata_42 |>
  tab_tidydata() |>
  tab_spanner(label = "Bevölkerung", columns = c("1999", "2000"))
```
...
1. Jeder Beobachtung entspricht einer Zeile.
...
In den folgenden Abschnitten lernen wir einige in der Statistik übliche Begriffe kennen, die im Zusammenhang mit statistischen Datensätzen verwendet werden. Eine ausführliche Darstellung findet sich zum Beispiel in @fahrmeir_statistik_2016.
...
#| fig-alt: |
#|  169 Beobachtungen mit zwei verschiedenen Stichproben
#| fig-cap: |
#|  Grundgesamtheit und Stichproben. Das Ergebnis hängt von der Auswahl der Stichprobe ab
...
- @fig-stichprobe zeigt zwei Stichproben. Welche der beiden Stichproben können wir verwenden, um Aussagen über die Verteilung der Farben in der Grundgesamtheit zu treffen? Wie groß ist dann groß genug? Mit diesen Fragen befasst sich die schließende Statistik.
...
::: {#def-statistische-grundbegriffe}
## Statistische Grundbegriffe
...
a. Auf welcher Skala wird ein Merkmal gemessen?

a. Beschreibt das Merkmal eine Qualität oder eine Größe, die in Zahlen messbar ist?
...
1. [Ordinalskala]{.neuerbegriff}: Wenn es für die Ausprägungen eines Merkmals eine natürliche Ordnung gibt, es aber keinen Sinn macht, Abstände zwischen den Werten zu bestimmen, dann ist das Merkmal ordinalskaliert.

1. [Intervallskala]{.neuerbegriff}: Man spricht von einem intervallskalierten Merkmal, wenn die Ausprägungen
...
1. [Verhältnisskala]{.neuerbegriff}: Gibt es für ein zahlenwertiges Merkmal zusätzlich noch einen natürlichen Nullpunkt, dann lassen sich Quotienten bilden und man spricht von einem verhältnisskalierten Merkmal. Zum Beispiel gibt es für die Geschwindigkeit einen natürlichen Nullpunkt und der Satz
...
Intervall- und Verhältnisskalen werden unter den Oberbegriffen [Kardinalskala]{.neuerbegriff} oder [metrische Skala]{.neuerbegriff} zusammengefasst. In der folgenden Tabelle sind Skalenniveaus und zugehörige sinnvolle Operationen nochmals kompakt dargestellt.
...
1. [Quantitative Merkmale]{.neuerbegriff}: Lässt sich ein Merkmal auf einer Intervall- oder Verhältnisskala messen, dann spricht man von einem quantitativen Merkmal. Eine Ausprägung ist hier eine Zahl, die eine Menge, Intensität oder ein Ausmaß wiedergibt.
