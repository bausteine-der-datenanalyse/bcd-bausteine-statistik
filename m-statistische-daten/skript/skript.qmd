...
#### Geschwindigkeitsmessung Universitätsstraße {#sec-beispiel-sdr}
...
ggplot(data = d_abfluss_donau) + 
  geom_point(
    ...
    alpha = 1/5, show.legend = FALSE, size = 0.1
  ) +
...
#| fig-cap: Grundgesamtheit und Stichproben
#| fig-alt: "169 Beobachtungen mit zwei verschiedenen Stichproben."
n <- 13.0
...
p1 <- ggplot(data = df) +
    geom_tile(
        data = samples, mapping = aes(x = x, y = y, width = s, height = s),
        fill = "light gray", color = "black", linewidth = 0.75
    ) +
    geom_point(
        mapping = aes(x = x, y = y, color = c), size = 2.5, show.legend = FALSE
    ) +
    annotate(geom="text", x = 0.5, y = n + 1.25, label = "Grundgesamtheit", hjust = "left") +
    annotate(geom="text", x = 10, y = n + 1.25, label = "Stichprobe A", hjust = "left") +
    annotate(geom="step", x = c(7.5, 7.5, 9.9), y = c(11.5, n + 1.25, n + 1.25)) +
    annotate(geom="text", x = 10, y = -0.25, label = "Stichprobe B", hjust = "right") +
    annotate(geom="step", x = c(10.1, 10.5), y = c(-0.25, 2.5)) +
    coord_fixed() +
    theme_void()
...
p22 <- plot_bars(df, "Stichprobe A")
p23 <- plot_bars(df, "Stichprobe B")
...
::: definition
**Definition: Statistische Grundbegriffe**
