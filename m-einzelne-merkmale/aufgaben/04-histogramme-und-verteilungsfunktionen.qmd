...
library(lubridate)
library(cowplot)
source(rprojroot::is_r_package$find_file("_bcd-setup.R"))
```

## Histogramme und Verteilungsfunktionen

Welche empirische Verteilungsfunktion (A-D) gehört zu welchem Histogramm (1-4)?
...
plot_grid(
  ...
  plot_ecdf(d4, "4"), plot_ecdf(d3, "3"), plot_ecdf(d2, "2"), plot_ecdf(d1, "1"),
  ncol = 4
)
```
