...
ggplot(data = d_klausur_m2) +
  geom_bar(mapping = aes(x = note_dezimal), width = 0.01) +
...
::: beispiel
**Beispiel Klausurergebnisse (Fortsetzung):** Mit den Klassen $$[0, 5), [5, 10), \dots, [95, 100)$$
...
ggplot(data = d_klausur_m2) +
  geom_histogram(
    mapping = aes(x = note_ganz, fill = bestanden),
    binwidth = 5, boundary = 0, closed = "left", show.legend = FALSE
  ) +
...
sx <- scale_x_continuous(
  breaks = c(1.1, 1.3, 1.9, 2.1, 2.5, 4),
  limit = c(0.8, 4.3),
  minor_breaks = NULL
)
...
p2 <- ggplot(data = d_hfkv_2) +
  ...
  geom_point(
    data = filter(d_hfkv_2, Xs > d_hfkv_a),
    mapping = aes(x = Xs, y = S)
  ) +
  geom_point(
    data = filter(d_hfkv_2, Xe < d_hfkv_b),
    mapping = aes(x = Xe, y = S),
    shape = 21
  ) +
  sx +
...
```{r}
d_h1 <- tibble(x = c(1.1, 1.3, 1.9, 2.5, 2.1, 1.3, 4))
d_h2 <- d_h1 |>
  rename(a = x) |>
  arrange(a) |>
  group_by(a) |>
...
ggplot(data = d_klausur_bestanden) +
  ...
  geom_path(
    ...
    mapping = aes(x = x, y = y),
    color = 'hotpink1',
    arrow = arrow(angle = 10, ends = "last", type = "open"),
    linewidth = 1.25
  ) +
