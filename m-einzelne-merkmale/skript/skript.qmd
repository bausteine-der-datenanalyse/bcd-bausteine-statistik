...
::: {#def-absolute-und-relative-häufigkeit}
## Absolute und relative Häufigkeit
Für die Werte eines Merkmals $x_1, x_2, \dots, x_n$ mit den unterschiedlichen Ausprägungen $a_1, a_2, \dots, a_k$ verwenden wir die Bezeichnungen
...
#| fig-cap: "Gebräuchliche Diagrammtypen (Stab-, Säulen-, Balken-, Kreis- und Ringdiagramm)
#|           zur Darstellung von Verteilungen (Energieträger Bundesrepublik Deutschland 2017,
#|           Datenquelle: BMWI)"

d <- import_energietrager("A21:AC30") |> filter(Jahr == 2017)

p_col <- ggplot(data = d) +
  geom_col(
    mapping = aes(x = Energieträger, y = Wert),
    width = 0.125
  ) +
  labs(
    x = NULL,
...
p_bar <- ggplot(data = d) +
  geom_col(
    mapping = aes(x = Energieträger, y = Wert, fill = Energieträger),
    show.legend = FALSE
  ) +
  scale_fill_manual(
    values = c_energietraeger
  ) +
  labs(
    x = NULL,
    y = "Anteil in Prozent"
...
p_pie <- ggplot(data = d) +
  geom_col(
    mapping = aes(x = 2, y = Wert, fill = Energieträger),
    width = 1
  ) +
  coord_polar(
    start = 0,
...
free(p1 + p2 + plot_layout(axes = "collect")) /
  free(p3) /
...
: Zahlenwerte für $h_1, h_2, \dots, h_k$ (oder $f_1, f_2, \dots, f_k$) werden mit einem senkrechten Strich angetragen.
...
- Die Darstellung der zeitlichen Entwicklung einer Verteilung wie in @fig-diagramm-balken-jahre ist nicht möglich.
...
#| fig-cap: "Zeitliche Entwicklung einer Verteilung in einem Säulendiagramm
#|           (Energieträger Bundesrepublik Deutschland 1990 - 2017, Quelle: BMWI)"

ggplot(data = import_energietrager("A8:AC17")) +
...
#| fig-cap: "Drei Verteilungen jeweils in einem Kreis- und einem Säulendiagramm dargestellt.
#|           Die Kreisdiagramme unterscheiden sich auf den ersten Blick nur unwesentlich,
#|           die Eigenschaften der Verteilungen sind am Säulendiagramm besser ablesbar"

set.seed(1)
d <- tibble(n = LETTERS[4:1], x1 = c(16, 17.5, 19, 21.5), x2 = sample(x1), x3 = rev(x1))

plot_pie <- function(v) {
  ggplot(data = d, mapping = aes(x = "", y = {{ v }}, fill = n, label = n)) +
...
plot_col <- function(v) {
  ggplot(data = d, mapping = aes(x = n, y = {{ v }}, fill = n)) +
    geom_col(show.legend = FALSE) +
...
plot_pie(x1) + plot_pie(x2) + plot_pie(x3) + plot_col(x1) + plot_col(x2) + plot_col(x3)
```
...
ggplot(data = d_klausur) +
  geom_bar(mapping = aes(x = prozent), width = 0.01) +
...
1. Für die [Gruppierung]{.neuerbegriff} der Daten wählen wir als [Klassen]{.neuerbegriff} die $k$ benachbarten Intervalle 
$$
...
ggplot(data = d_klausur) +
  geom_histogram(
    mapping = aes(x = note, fill = bestanden),
    binwidth = 5,
    boundary = 0,
    closed = "left",
    show.legend = FALSE
...
set.seed(432)
rnorm(15000) |> p_histogram()
c(rnorm(3500), rnorm(1500, 4)) |> p_histogram()
c(rnorm(3500), rnorm(1500, 4), rnorm(2500, 10)) |> p_histogram()
```
...
#| fig-alt: "Gegenüberstellung einer linkssteilen, einer symmetrischen und einer
#|           rechtssteilen Verteilung"

rlnorm(10000) |> p_histogram(q = 0.95)
rnorm(10000) |> p_histogram()
rlnorm(10000) |> p_histogram(q = 0.95) + scale_x_reverse()
```
...
```{r}
d1 <- tibble(a = c(1.1, 1.3, 1.9, 2.5, 2.1, 1.3, 4)) |>
  arrange(a) |>
  group_by(a) |>
  summarize(h = n()) |>
  mutate(
...
d1 |>
  ...
  cols_label(
    a = md("Ausprägung $a_i$"),
    h = md("Häufigkeit $h_i$"),
    H = md("Kumulierte Häufigkeit $H_i$")
  ) |>
  cols_align("center") |>
  tab_stubhead(label = md("$i$")) |>
...
d2 <- d1 |>
  rename(start = a) |>
  add_row(start = 0.8, H = 0, .before = 1) |>
  mutate(end = lead(start, default = 4.3))

sx <- scale_x_continuous(
  breaks = pull(d1, a),
  limit = c(min(pull(d2, start)), max(pull(d2, end))),
  minor_breaks = NULL
)

p1 <- ggplot(data = d1) +
  geom_col(mapping = aes(x = a, y = h)) +
  sx +
  scale_y_continuous(breaks = 0:2, minor_breaks = NULL) +
  labs(x = NULL, y = NULL)

p2 <- ggplot() +
  geom_segment(data = d2, mapping = aes(x = start, xend = end, y = H)) +
  geom_point(data = d2 |> tail(n = -1), mapping = aes(x = start, y = H)) +
  geom_point(data = d2 |> head(n = -1), mapping = aes(x = end, y = H), shape = 21) +
  sx +
...
```{r}
#| fig-alt: "Treppenfunktion der erreichten Prozentpunkte. Der Funktionswert $F(49)$
#|           entspricht den 32% der Teilnehmenden, die nicht bestanden haben."

f <- ecdf(pull(d_klausur, note))
pp <- tibble(x = c(0, 49), y = c(f(49), 0))

ggplot(data = d_klausur) +
  geom_step(data = pp, mapping = aes(x = x, y = y), linewidth = 1, color = "hotpink1") +
  geom_step(mapping = aes(x = note), stat = "ecdf", linewidth = 1) +
  scale_x_continuous(breaks = c(0, 25, 50, 75, 100), minor_breaks = -10:110) +
  scale_y_continuous(breaks = c(0, round(f(49), 2), 1), minor_breaks = NULL) +
