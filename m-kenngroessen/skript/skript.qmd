...
p_mittelwert <- function(d, m) {
  ...
  ggplot(data = d) +
    geom_dotplot(
      mapping = aes(x = {{m}}),
      method = "histodot", dotsize = 0.75, binwidth = 1, stackratio = 1.25
    ) +
    ...
    annotate("text", x = x_bar + 0.3, y = 0.5, label= "Arithmetisches Mittel",  hjust = "left", vjust = "center", size = 6) +
    labs(x = NULL, y = NULL)
}
```
...
ggplot(data = d_bsp_gm_w, mapping = aes(x = Tag, y = Wasserstand)) +
  ...
  geom_point(size = 3) +
  scale_x_continuous(breaks = 0:nrow(d_bsp_gm_w), minor_breaks = NULL) +
  annotate("text", x = 2.0, y = 3, label = "Wachstumsfaktor", hjust = "right", vjust = "top") +
  annotate("text", x = 2.8, y = 4.5, label = "Wasserstand", hjust = "right") +
  labs(x = "Tag", y = NULL)
```
...
ggplot(data = d_bsp_gm_wm, mapping = aes(x = Tag, y = Wasserstand)) +
  ...
  geom_line(data = d_bsp_gm_w, linewidth = 1.15, alpha = 0.2) +
  geom_line(linewidth = 1.15) + 
  geom_point(size = 3) +
  scale_x_continuous(breaks = 0:nrow(d_bsp_gm_w), minor_breaks = NULL) +
  scale_fill_discrete(palette = scales::hue_pal(direction = -1)(2)) +
  annotate(
...
Im Jahr 2010 titelte der Spiegel ["So verrückt war dieser Sommer"](http://www.spiegel.de/wissenschaft/natur/extremwetter-so-verrueckt-war-dieser-sommer-a-714629.html): Noch nie hat es seit Beginn der Aufzeichnungen 1881 im August so viel geregnet. Demgegenüber war es im Juli noch sehr trocken, einen Vergleich aus der Stadt Dresden zeigt @fig-dresden. 

::: {#fig-dresden layout-ncol=2}
![](00-bilder/sommer-2010-1.png){fig-alt="Elbe mit sehr niedrigem Wasserstand"}

![](00-bilder/sommer-2010-2.png){fig-alt="Elbe mit Hochwasser"}

Die Bilder (Quelle: spiegel.de) zeigen, wie sich das extreme Wetter des Jahres 2010 in Dresden auswirkte. Die Elbe führt im Juli fast kein Wasser (links), nur um kurz darauf im August über die Ufer zu treten (rechts).
:::
...
ggplot(
  ...
  geom_line(linewidth = 1.15) + 
  geom_point(size = 3) +
  labs(x = NULL, y = "Monatlicher Niederschlag in mm")
```
...
::: beispiel
In @fig-konzentration-niederschlag sind die monatlichen Niederschläge des Jahres 2008 in Bochum und des Jahres 2010 in Nha Trang (Vietnam) zu sehen. Insgesamt sind in Bochum $\SI{869}{mm}$ und in Nha Trang $\SI{`r sum(pull(d_ns_nt_2015_monat, Niederschlag))`}{mm}$ Niederschläge gefallen. Allerdings konzentrieren sich die Niederschläge in Vietnam auf drei Monate, in allen anderen Monaten regnet es sogar weniger als in Bochum. Eine solche Konzentration von großen Beiträgen zu einer Gesamtsumme auf wenige Merkmalen wird mithilfe eines Konzentrationsmaßes quantifiziert.

```{r}
#| label: fig-konzentration-niederschlag
#| fig-cap: "Monatliche Niederschläge in Bochum (2008) und dem vietnamesischen Badeort Nha Trang (2010). Quellen: DWD und Behörde VN"
#| fig-alt: "Monatliche Niederschläge in Bochum und Nha Trang in einem Balkendiagramm. Der Niederschlag in Nha Trang konzentriert sich auf den Zeitraum von Oktober bis Dezember."
...
$$
= 2 \cdot \text{Fläche zwischen Diagonale und Lorenzkurve} \text{.}
$$
:::
