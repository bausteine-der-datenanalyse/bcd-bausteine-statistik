...
p1 <- plot_lageregeln(d_bsp_lageregeln, "I")
p2 <- plot_lageregeln(d_bsp_lageregeln, "II")
p3 <- plot_lageregeln(d_bsp_lageregeln, "III")
...
$$
\begin{aligned}
\text{Stichprobe I} & \quad \text{linkssteil} & \quad \bar{x} > x_\mathrm{med} > x_\mathrm{mod} \\
\text{Stichprobe II} & \quad \text{symmetrisch} & \quad \bar{x} = x_\mathrm{med} = x_\mathrm{mod} \\
\text {Stichprobe III} & \quad \text{rechtssteil} & \quad  \bar{x} < x_\mathrm{med} < x_\mathrm{mod}
\end{aligned}
$$
...
$$
\begin{aligned}
\text{Linkssteil} \quad \iff \bar{x} > x_\mathrm{med} > x_\mathrm{mod} \\
\text{Symmetrisch} \quad \iff \bar{x} \approx x_\mathrm{med} \approx x_\mathrm{mod} \\
\text{Rechtssteil} \quad \iff \bar{x} < x_\mathrm{med} < x_\mathrm{mod}
\end{aligned}
$$
:::
...
+-----------------------+--------+--------+--------+--------+--------+------------------+
| Wachstumsfaktor $x_i$ | 2.00   | 0.75   | 3.00   | 0.50   | 2.00   | 1.11             |
+-----------------------+--------+--------+--------+--------+--------+------------------+
...
Für einen mittleren Wachstumsfaktor $z$ müssen wir dasselbe Ergebnis erhalten, wenn wir sechs Tage lang diesen Wachstumsfaktor ansetzen. Es muss daher

$$
W_6 = z \cdot z \cdot z \cdot z \cdot z \cdot z \cdot W_0 = z^6 \cdot W_0
$$

gelten, so dass wir den mittleren Wachstumsfaktor
...
$$ 
  B_n = \underbrace{\bar{x}_\mathrm{geom} \cdot \; \dots \; \cdot \bar{x}_\mathrm{geom}}_{n-\text{mal}} \cdot B_0 = (\bar{x}_\mathrm{geom})^n B_0.
$$ 
...
\sum_{i=1}^n(x_i - \bar{x})
  ...
  =
  n\bigg(\underbrace{\frac{1}{n}\sum_{i=1}^n x_i}_{\bar{x}} - \bar{x}_{mid} \bigg) = 0.
$$
...
2.  Ein Strich in dem Rechteck markiert die Lage des Medians $x_\mathrm{med}$.
3.  Alle Werte im Intervall $[x_{0.25} - 1.5 d_\mathrm{Q}, x_{0.75} + 1.5 d_\mathrm{Q}]$ werden als normale Beobachtungen angesehen.
4.  Zwei Linien (die [*whiskers*]{.eng}) gehen bis zur kleinsten und bis zur größten normalen Beobachtung. Manchmal werden diese Linien durch einen kurzen Querstrich abgeschlossen.
...
## Konzentrationsmaß {#sec-konzentrationsmaß}
...
::: definition
**Definition: normierter Gini-Koeffizient/Lorenz-Müller-Koeffizient**
