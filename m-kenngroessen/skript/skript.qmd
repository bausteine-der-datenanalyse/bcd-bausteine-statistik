...
1. Wenn man sich vorstellt, dass jedem Wert $x_j$ eine Kugel entspricht, die an der Stelle $x_j$ an eine gewichtslose Stange gehängt wird, dann ist $\bar{x}$ die Stelle, an der man die Stange auf einem Finger balancieren kann (also der Schwerpunkt).
...
ggplot(data = data) +
  ...
  geom_segment(
    data = threads,
    mapping = aes(x = X, xend = X, y = bar_y + ticklength, yend = Y),
    linewidth = 0.35
  ) +
  geom_dotplot(
    mapping = aes(x = X),
...
$$
\begin{aligned}
\bar{x} & = \frac{1}{17}(1 + 2 + 6 + \dots + \phantom{1}2 + \dots + 2) = 2.588\dots \\[1em]
...
mtitle <- function(l) {
  annotate(geom = "text", label = l, x = 21, y = 1, hjust = "right", vjust = "top")
}

p1 <- p_mittelwert(d_bsp_mittelwert, X) + mtitle("Merkmal X")
p2 <- p_mittelwert(d_bsp_mittelwert, Y) + mtitle("Merkmal Y")
...
d_bsp_lageregeln |>
  ...
  grand_summary_rows(
    ...
    fmt = ~ fmt_number(., drop_trailing_zeros = TRUE)
  ) |>
...
$$
\begin{aligned}
\text{linkssteil} & \iff \bar{x} > x_\mathrm{med} > x_\mathrm{mod} \\
\text{symmetrisch} & \iff \bar{x} \approx x_\mathrm{med} \approx x_\mathrm{mod} \\
\text{rechtssteil} & \iff \bar{x} < x_\mathrm{med} < x_\mathrm{mod}
\end{aligned}
$$
:::
...
ggplot(data = d_reservoir_w, mapping = aes(x = Tag, y = Wasserstand)) +
  geom_col(
    data = d_reservoir_x,
  ...
  scale_x_continuous(
    breaks = 0:nrow(d_reservoir_w), minor_breaks = NULL
  ) +
  annotate(
...
Die Definition des Wachstumsfaktors stellen wir zu $W_i = x_i W_{j-i}$ um. Mit den Wachstumsfaktoren $x_i$ und dem Wasserstand $W_0 = 1$ können wir also den Verlauf des Wasserstandes für die betrachteten Tage folgendermaßen rekonstruieren:
$$
\begin{aligned}
W_1  
& = 
...
w0 <- pluck(d_reservoir_w, "Wasserstand", 1)
xm <- pull(d_reservoir_x, "Wachstumsfaktor") |> mean_geom()

d_reservoir_w |>
  mutate(Wasserstand = w0 * xm^Tag) |>
  ggplot(mapping = aes(x = Tag, y = Wasserstand)) +
  geom_col(
    data = d_reservoir_x,
    mapping = aes(x = Zeitraum - 0.5, y = xm, fill = TRUE),
    alpha = 0.5,
    show.legend = FALSE
  ) +
  geom_line(
  ...
  annotate(
    ...
    x = 6,
    y = 1.5,
    label = "Mittlerer Wachstumsfaktor",
    hjust = "right",
    vjust = "bottom"
  ) +
  annotate(
...
$$ 
Mit $\bar{x}_\mathrm{geom}$ als mittlerem Wachstumsfaktor für alle Perioden ermittelt man also denselben Bestand $B_n$, der sich auch mit den tatsächlichen Wachstumsfaktoren $x_1, \dots, x_n$ ergibt. In diesem Sinn ist $\bar{x}_\mathrm{geom}$ die adäquate Mittlung der Wachstumsfaktoren.
...
::: beispiel
**Beispiel Autofahrt:** Bei einer Autofahrt werden zunächst $\SI{10}{km}$ mit einer Geschwindigkeit von $\SI{50}{km/h}$ zurückgelegt, danach weitere $\SI{60}{km}$ bei $\SI{110}{km/h}$. Wie groß ist die Durchschnittsgeschwindigkeit?
...
d_ns_bo_2008_2010_monat |>
  filter(Monat <= month(6, label = TRUE)) |>
  tab_niederschlag()
```

```{r}
d_ns_bo_2008_2010_monat |>
  filter(Monat >= month(7, label = TRUE)) |>
  tab_niederschlag()
```

```{r}
#| fig-alt: |
# Monatliche Niederschläge in Bochum für die Jahre 2008 und 2012.
# Die Mittelwerte unterscheiden sich nur geringfügig.
...
ggplot(
  ...
  labs(
    x = NULL,
    y = "Monatlicher Niederschlag in mm"
  )
```
...
\bar{x}_{2008} 
& = 
\frac{1}{12}(91.8 + 48.9 + \cdots + 43.9) =
\SI{`r pluck(d_niederschlag_kenngroessen, "Mittelwert", 1)`}{mm} \\
\bar{x}_{2010} 
& = 
\frac{1}{12}(48.6 + 73.0 + \cdots + 84.8) = 
\SI{`r pluck(d_niederschlag_kenngroessen, "Mittelwert", 2)`}{mm}
\end{aligned}
$$
...
$$
\begin{aligned}
\tilde{s}_{2008} & = \sqrt{453.17} = \SI{21.29}{mm} \\[1em]
...
::: {#def-quantil}
## Quantil
Die Zahl $x_p$ mit $0 < p < 1$ heißt $p$-Quantil einer Stichprobe, wenn mindestens ein Anteil von $p \cdot$ 100% der Daten kleiner-gleich $x_p$ und mindestens ein Anteil von $(1-p) \cdot$ 100% größer-gleich $x_p$ ist. Es muss für $x_p$ also
$$
...
$$
heißt Interquartilsabstand. Zusammen mit den Extremwerten und dem Median erhalten wir die [Fünf-Punkte-Zusammenfassung]{.neuerbegriff} einer Verteilung
$$
...
ggplot(data = d) +
  ...
  labs(
    x = NULL,
    y = NULL,
    title = "Monatliche Niederschläge in mm"
  ) +
  coord_cartesian(
    xlim = c(0, 3)
  )
...
ggplot(data = d_ns_bo_2008_2010) +
  ...
  labs(
    x = NULL,
    y = NULL,
    title = "Tägliche Niederschläge in mm"
  )
```
:::
...
# (Daten: [Weltbank](https://data.worldbank.org/indicator/SI.POV.GINI))
...
d <- d_ns_bo_nt |>
  group_by(Ort) |>
  arrange(Ort, Niederschlag) |>
  mutate(Rang = row_number())

d |>
  filter(Rang <= 6) |>
  tab_niederschlag_sortiert()
```
...
#| fig-asp: 0.55
#| fig-alt: |
# Lorenzkurven für die Niederschläge in Bochum und Nha Trang. Der Bauch der Kurve ist für
# Nha Trang deutlich größer als für Bochum.
...
```{r}
#| fig-asp: 0.55
#fig-alt: "Grenzfälle für den Gini-Koeffizienten:
#          Links sind alle Werte gleich, rechts nur ein Wert ungleich null."
...
## Normierter Gini-Koeffizient oder Lorenz-Münzer-Koeffizient
...
sx <- scale_x_continuous(breaks = c(0, 0.5, 1), minor_breaks = NULL)
sy <- scale_y_continuous(breaks = c(0, 0.1, 0.5, 1), minor_breaks = NULL)

p1 <- p_lorenzkurve(d_bsp_gini_reduktion, "A", "") + sx + sy
p2 <- p_lorenzkurve(d_bsp_gini_reduktion, "B", "") + sx + sy
